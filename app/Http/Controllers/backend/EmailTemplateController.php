...
class EmailTemplateController extends CommonController{
    ...
    protected $slug         =   'email_templates';
    ...
    public function edit($id,Request $request){
        if($request->isMethod('post')){
            ...
            $exists_error = [];
            $exists   =   EmailTemplate::where("id","!=",base64_decode($id))->where("title",$request['title'])->where("is_delete",0)->get();
    ...
    public function create(Request $request){
        if($request->isMethod('post')){
            ...
            /** Check Exists Work Start **/
            $exists_error = [];
            $exists   =   EmailTemplate::where("title",$request['title'])->where("is_delete",0)->get();
            if($exists->count() > 0){ $exists_error["title"]   =  ["The title has already been taken"];  }
            if(!empty($exists_error)){
                return redirect()->back()->withInput()->withErrors($exists_error);
            }
